	...
	<div class="row">
			...
			<div class="card card-info card-primary">
				...
				<div class="card-footer">
					<div class="d-flex justify-content-end w-100">
						<button class="btn btn-sm btn-primary btn-flat bg-gradient-primary mx-1" form="manage-question">Lưu</button>
						<button class="btn btn-sm btn-flat btn-secondary bg-gradient-secondary mx-1" form="manage-question" type="reset">Hủy</button>
					</div>
				</div>
			</div>
		</div>
		<div class="col-md-8">
			<div class="card card-outline card-info">
				<div class="card-header">
					<b>Bảng đánh giá câu hỏi cho khóa học: <?php echo $_SESSION['academic']['year'].' - Học kì '.(ordinal_suffix($semester)) ?> </b>
					<div class="card-tools">
						<button class="btn btn-sm btn-flat btn-primary bg-gradient-primary mx-1" id="eval_restrict" type="button">Giới hạn đánh giá</button>
						<button class="btn btn-sm btn-flat btn-success bg-gradient-success mx-1" form="order-question">Lưu</button>
					</div>
				</div>
				<div class="card-body">
					<fieldset class="border border-info p-2 w-100">
					   <legend  class="w-auto">Đánh Giá</legend>
					   <p>5 = Hoàn toàn tốt;   4 = Tốt;   3 = Tệ;   2 = Hoàn toàn tệ;   1 = Không ý kiến</p>
					</fieldset>
								...
								<td class="p-1 text-center" width="5px">
									<span class="btn-group dropright">
									  ...
									  <div class="dropdown-menu">
									     <a class="dropdown-item edit_question" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>">Chỉnh sửa</a>
					                      <div class="dropdown-divider"></div>
					                     <a class="dropdown-item delete_question" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>">Xóa</a>
									  </div>
									</span>
								</td>
...
<script>
	...
	$('.delete_question').click(function(){
		_conf("Bạn có chắc chắn xóa câu hỏi này không?","delete_question",[$(this).attr('data-id')])
		})
	$('#eval_restrict').click(function(){
		uni_modal("Quản lý các hạn chế đánh giá","<?php echo $_SESSION['login_view_folder'] ?>manage_restriction.php?id=<?php echo $id ?>","mid-large")
	})
    ...
    $('#manage-question').submit(function(e){
    	...
    	start_load()
    	if($('#question').val() == ''){
    		alert_toast("Vui lòng điền vào mô tả câu hỏi trước",'error');
    		end_load();
    		return false;
    	}
    	$.ajax({
